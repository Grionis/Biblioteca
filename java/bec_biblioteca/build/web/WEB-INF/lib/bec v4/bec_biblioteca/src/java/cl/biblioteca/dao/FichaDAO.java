...
public class FichaDAO {
    ...
    public List<cl.biblioteca.edm.Ficha> getList() throws Exception {
        ...
        try 
        {
            ...
            List<cl.biblioteca.edm.Ficha> lista =  (List<cl.biblioteca.edm.Ficha>) session.createCriteria(cl.biblioteca.edm.Ficha.class).list(); 
    ...
    public cl.biblioteca.edm.Ficha getFichaById(int id) throws Exception
    {
         Session session = HibernateUtil.getSessionFactory().openSession();
        try {
            session.beginTransaction();
            ...
            cl.biblioteca.edm.Ficha tmp = (cl.biblioteca.edm.Ficha) session.createCriteria(cl.biblioteca.edm.Ficha.class)
                    .add(Restrictions.eq("idFicha", id))
                    .uniqueResult();
            session.getTransaction().commit();
            session.close();
            return tmp;
